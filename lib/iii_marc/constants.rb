module MARC
  ...
  URI_FOR_RECORD = '/record=%s~S%s'
  URI_FOR_MARC = '/search~S%s/.%s/.%s/1%%2C1%%2C1%%2CB/marc~%s'
  MARC_REGEX = Regexp.new('<pre>(.+)</pre>', Regexp::MULTILINE)
  
  URI_FOR_KEYWORD_SEARCH = '/search~S%s/?searchtype=X&searcharg=%s'
  URI_FOR_TITLE_SEARCH = '/search~S%s/?searchtype=t&searcharg=%s'
  
  RECORD_TYPES = {
    'BKS' => MARC::BookRecord,
    'SER' => MARC::SerialRecord, 
    'VIS' => MARC::VisualRecord, 
    'MIX' => MARC::MixedRecord, 
    'MAP' => MARC::MapRecord, 
    'SCO' => MARC::ScoreRecord, 
    'REC' => MARC::SoundRecord, 
    'COM' => MARC::ComputerRecord
  }
  ...
  KEYWORD_STOPWORDS = [
    ...
    "course", "currently", "definitely", "described", "despite", "did",
    "didn't", "different", "do", "does", "doesn't", "doing", "don't",
    "done", "down", "downwards", "during", "each", "edu", "eg", "eight",
    ...
    "regards", "relatively", "respectively", "right", "said", "same", "saw",
    ...
    "wonder", "would", "would", "wouldn't", "yes", "yet", "you", "you'd",
